...
const FormResponse: React.FC<FormResponseProps> = ({ form, onClose }) => {
  ...
  // Group fields by sections with unique IDs
  const sections = form.fields.reduce((acc: any[], field, index) => {
    const sectionId = `section-${index}`;
    if (field.type === 'section') {
      acc.push({
        id: sectionId,
        title: field.label,
        fields: field.fields?.map((f, i) => ({
          ...f,
          uniqueId: `${sectionId}-field-${i}`
        })) || []
      });
    } else {
      if (acc.length === 0) {
      ...
      acc[acc.length - 1].fields.push({
        ...field,
        uniqueId: `${acc[acc.length - 1].id}-field-${acc[acc.length - 1].fields.length}`
      });
    }
    return acc;
  }, []);
  ...
  const renderField = (field: any) => {
    ...
    switch (field.type) {
      ...
      case 'scale':
        const config = field.scaleConfig || { min: 1, max: 5, step: 1, showValue: true };
        return (
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm text-gray-600">
              ...
              {config.showValue && (
                <span className="font-medium">Valor: {value ?? config.min}</span>
              )}
        ...
        <div className="flex space-x-2">
          {sections.map((section) => (
            <button
              key={section.id}
              type="button"
              onClick={() => setCurrentSection(sections.indexOf(section))}
              className={`w-3 h-3 rounded-full ${
                sections.indexOf(section) === currentSection ? 'bg-blue-600' : 'bg-gray-300'
              }`}
            />
          ))}
        </div>
