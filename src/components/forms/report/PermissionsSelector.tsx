      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="mt-2 space-y-2">
            {users.map((user) => (
              <label key={`user-${user.id}`} className="flex items-center">
                <input
                  type="checkbox"
                  checked={permissions.users.includes(user.id)}
                  onChange={() => {
                    const newUsers = permissions.users.includes(user.id)
                      ? permissions.users.filter(id => id !== user.id)
        ...
        <div>
          ...
          <div className="mt-2 space-y-2">
            {subnets.map((subnet) => (
              <label key={`subnet-${subnet.id}`} className="flex items-center">
                <input
                  type="checkbox"
                  checked={permissions.subnets.includes(subnet.id)}
                  onChange={() => {
                    const newSubnets = permissions.subnets.includes(subnet.id)
                      ? permissions.subnets.filter(id => id !== subnet.id)
                      : [...permissions.subnets, subnet.id];
                    onPermissionsUpdated({
                      ...permissions,
                      subnets: newSubnets,
                    });
        ...
        <div>
          ...
          <div className="mt-2 space-y-2">
            {roles.map((role) => (
              <label key={`role-${role.id}`} className="flex items-center">
                <input
                  type="checkbox"
                  checked={permissions.roles.includes(role.id)}
                  onChange={() => {
                    const newRoles = permissions.roles.includes(role.id)
