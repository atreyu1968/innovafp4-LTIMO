...
const Navbar: React.FC<NavbarProps> = ({ className = '' }) => {
  ...
  const handleLogout = () => {
    // Prevenir cierre de sesión si es admin y hay mantenimiento activo
    if (user?.role === 'coordinador_general' && 
        activeRole === 'admin' && 
        settings.maintenance?.enabled) {
      alert('No puedes cerrar sesión mientras el sistema está en mantenimiento');
      return;
    }
    logout();
  };
  ...
  return (
    <nav className={`h-16 ${className}`} style={{ backgroundColor: settings.colors.navbar.from }}>
      <div className="h-full flex items-center px-4">
        ...
        {activeYear && (
          <Link 
            to={user?.role === 'coordinador_general' ? '/cursos' : '#'}
            className={`ml-8 flex items-center text-white/90 ${user?.role === 'coordinador_general' ? 'hover:text-white' : ''}`}
          >
            <Calendar className="h-4 w-4 mr-2" />
            <span className="text-sm font-medium">Curso {activeYear.year}</span>
          </Link>
        )}
