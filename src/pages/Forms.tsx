...
const Forms = () => {
  ...
  const { forms, addForm, updateForm, deleteForm, canUserRespond } = useFormStore();
  ...
  const handleFormClick = (form: Form) => {
    if (!isAdmin) {
      ...
      if (!canUserRespond(user?.id || '', form.id)) {
        if (form.status === 'cerrado') {
          showNotification('error', 'Este formulario está cerrado y no acepta respuestas');
        } else if (!form.acceptingResponses) {
          showNotification('error', 'Este formulario no está aceptando respuestas en este momento');
        } else {
          showNotification('error', 'No puedes responder a este formulario');
        }
        return;
      }
    }
    ...
    setShowBuilder(isAdmin);
    setShowResponse(!isAdmin);
  };
